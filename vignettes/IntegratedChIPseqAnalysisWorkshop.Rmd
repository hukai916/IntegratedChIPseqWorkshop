...
Convert from bed/broadPeak/narrowPeak etc. to `GRanges` with the `toGRanges`.
...
Depending on your research, you can use either ENSEMBL or UCSC annotations.

The more complex ENSEMBL annotation is preferred when the goal is to discover and explain unknown biological mechanisms.

The less copmlex UCSC annotation can be used to generate more robust and reproducible results.
```{r "TxDbAnnoData"}
library(TxDb.Hsapiens.UCSC.hg19.knownGene)
...
Be careful that peak annotations may differ depending on the source of your annotation file.
...
The `assignChromosomeRegion` function summarizes the distributio of peaks over various features (exon, intron, enhancer, promoter and UTR).
...
## Example4: obtain enriched GO adn pathways {-#my-section}
...
## Example6: find peaks with bi-directional promoters {-#my-section}
...
## Example8: metagene analysis for given feauture/peak range {-#my-section}
...
#### Step1: prepare the methylation/variant/mutation data. {-#my-section}
```{r "quickStartStep1"}
library(trackViewer)
